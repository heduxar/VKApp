...
class UserGroupsTableViewController: UITableViewController {
    ...
    private lazy var myGroups = try? RealmProvider.get(Group.self).filter("member == %@", 1)
...
//        }) ?? false
        let member = myGroups?.contains {myGroups in return myGroups.name == newGroup?.name} ?? false
...
//                self.updateSections(deletions: deletions, insertions: insertions, modifications: modifications)
        ...
        if editingStyle == .delete {
            ...
            DispatchQueue.global().async {
                self.networkService.leaveGroup(groupId: groupId){[weak self]
                    ...
                    let group = try? RealmProvider.get(Group.self).filter("id == %@", id)
                    if response > 0 {
                        try? Realm().write {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        //MARK: - Commented = sepia for images
...
//        } else if let photo = myGroups?[indexPath.row] {
            cell.configureGroup(with: myGroups?[indexPath.row])
