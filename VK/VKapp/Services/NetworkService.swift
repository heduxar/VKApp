...
class NetworkService {
    ...
    //MARK: News
    func getNews(startTime: Double? = nil, startFrom: String? = nil, complition: @escaping (Result<(news: [News], users: [User], groups: [Group], nextFrom: String)>) -> Void){
        ...
        NetworkService.session.request(self.baseUrl + path, method: .get, parameters: params).responseJSON(queue: DispatchQueue.global()) { response in
            ...
            case .success(let value):
                ...
                newsDispatchGroup.notify(queue: .main){
                    let result = Result<(news: [News], users: [User], groups: [Group], nextFrom: String)>.success((news, profiles, groups, nextFrom))
                    complition (result)
                }
            case .failure(let error):
                print(error)
                let result = Result<(news: [News], users: [User], groups: [Group], nextFrom: String)>.failure(error)
                complition(result)
            }
        }
    }
